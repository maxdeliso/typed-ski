...
def _trip_llvm_object_impl(ctx):
    ...
    ctx.actions.run(
        ...
        progress_message = "Compiling %s to COFF object" % llvm_ir.short_path,
        use_default_shell_env = True,
    )
...
def _trip_executable_stdout_test_impl(ctx):
    ...
    if not is_windows:
        fail("trip_executable_stdout_test is Windows-only; add target_compatible_with to the target.")
