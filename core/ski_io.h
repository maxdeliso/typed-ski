...
/* Parse topoDagWire format: fixed-width records `term(3)left(8)right(8)`
 * separated by `|`, with child pointers encoded as hex byte offsets into the
 * serialized string. Leaves use `FFFFFFFF` null pointers. The last record is
 * the root. Returns root node id or EMPTY on error. *end_idx is set past the
 * consumed input. */
uint32_t parse_dag(const char *buf, size_t len, size_t *end_idx);

/* Serialize arena DAG to topoDagWire format using topological postorder. Only
