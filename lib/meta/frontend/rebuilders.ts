...
import { referencesVar, substituteSystemFType as substituteType } from '../../types/systemF.js';
...
export function polyRebuild(n: SystemFTerm, rebuilt: SystemFTerm[], term: TripLangTerm): SystemFTerm {
  if (isNonTerminalNode(n)) {
    ...
    const nextType = (term.kind === 'type' && referencesVar(n.typeArg, term.name))
...
export function typedRebuild(n: TypedLambda, rebuilt: TypedLambda[]): TypedLambda {
  if (isNonTerminalNode(n)) {
    return rebuildNonTerminal(n, rebuilt, cons);
...
export const untypedRebuild = (n: UntypedLambda, rebuilt: UntypedLambda[]): UntypedLambda =>
...
export function typedTypeRebuild(n: TypedLambda, rebuilt: TypedLambda[]): TypedLambda {
  if (isNonTerminalNode(n)) {
    return rebuildNonTerminal(n, rebuilt, cons);
  } else if (n.kind === 'typed-lambda-abstraction') {
    return rebuildTypedLambdaAbs(n, rebuilt);
