...
describe("externalReferences", () => {
  ...
  it("identifies external references in a term with nested abstractions", () => {
    ...
    assert.deepStrictEqual(Array.from(typeRefs.keys()).sort(), ["A", "B", "C"]);
  });

  it("identifies external references in a System F match expression", () => {
    const input =
    ...
    assert.deepStrictEqual(Array.from(termRefs.keys()).sort(), ["x", "y", "z"]);
    assert.deepStrictEqual(Array.from(typeRefs.keys()).sort(), ["T"]);
  });

  it("identifies external references in a System F match with nested match", () => {
    const input =
