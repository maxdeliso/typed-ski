...
describe(
  "Church numeral optimization functions",
  ...
  () => {
    ...
    it("optimization correctness", async () => {
      ...
      const testCases = [
        ...
        20, 21, 22, 23, 24, 25, 27, 30, 32, 36, 49, 64, 81, 100,
      ];

      for (const n of testCases) {
        const church = ChurchN(n);
        const decoded = await UnChurchNumber(
          await arenaEvaluator.reduce(church),
          arenaEvaluator,
        );
        assert.strictEqual(
          decoded,
    ...
    it("memoization", async () => {
      ...
      assert.strictEqual(
        await UnChurchNumber(
          await arenaEvaluator.reduce(church1),
          arenaEvaluator,
        ),
        10n,
      );
      assert.strictEqual(
        await UnChurchNumber(
          await arenaEvaluator.reduce(church2),
          arenaEvaluator,
        ),
        10n,
      );
      assert.strictEqual(
        await UnChurchNumber(
          await arenaEvaluator.reduce(church3),
          arenaEvaluator,
        ),
        10n,
      );
    ...
    it("optimization strategy verification", async () => {
      ...
      const decoded9 = await UnChurchNumber(
        await arenaEvaluator.reduce(church9),
        arenaEvaluator,
      );
      ...
      const decoded18 = await UnChurchNumber(
        await arenaEvaluator.reduce(church18),
        arenaEvaluator,
      );
      ...
      const decoded19 = await UnChurchNumber(
        await arenaEvaluator.reduce(church19),
        arenaEvaluator,
      );
      ...
      const decoded64 = await UnChurchNumber(
        await arenaEvaluator.reduce(church64),
        arenaEvaluator,
      );
    ...
    it("UnChurchNumber should handle Turner primes", async () => {
      ...
      const SPrime = {
        kind: "terminal" as const,
        sym: SKITerminalSymbol.SPrime,
      };
      const BPrime = {
        kind: "terminal" as const,
        sym: SKITerminalSymbol.BPrime,
      };
      const CPrime = {
        kind: "terminal" as const,
        sym: SKITerminalSymbol.CPrime,
      };

      assert.strictEqual(
        typeof (await UnChurchNumber(
          applyMany(SPrime, I, I, I),
          arenaEvaluator,
        )),
        "bigint",
      );
    ...
    it("UnChurchNumber with invalid church numeral (not returning bigint)", async () => {
      ...
      assert.strictEqual(
        await UnChurchNumber(invalidChurch, arenaEvaluator),
        0n,
      );
    });
  },
...
describe("Church encodings", { skip: !thanatosAvailable() }, () => {
  const N = 5;
  let arenaEvaluator: any;
  ...
  describe("succ / basic arithmetic", () => {
    ...
    it("1 + 1 = 2", async () => {
      assert.strictEqual(
        await UnChurchNumber(
          await arenaEvaluator.reduce(apply(Succ, ChurchN(1))),
          arenaEvaluator,
        ),
        2n,
      );
  ...
  it("pairs (make, fst, snd, car, cdr)", async () => {
    assert.strictEqual(
      await UnChurchNumber(
        await arenaEvaluator.reduce(applyMany(V, ChurchN(0), ChurchN(1), Fst)),
        arenaEvaluator,
      ),
      0n,
    );
    ...
    assert.strictEqual(
      await UnChurchNumber(
        await arenaEvaluator.reduce(applyMany(V, ChurchN(0), ChurchN(1), Snd)),
        arenaEvaluator,
      ),
      1n,
    );
  ...
  it("isZero predicate", async () => {
    ...
    assert.strictEqual(
      await UnChurchBoolean(
        await arenaEvaluator.reduce(
          applyMany(ChurchN(0), apply(K, False), True),
        ),
        arenaEvaluator,
      ),
      true,
    ...
    assert.strictEqual(
      await UnChurchBoolean(
        await arenaEvaluator.reduce(
          applyMany(ChurchN(1), apply(K, False), True),
        ),
        arenaEvaluator,
      ),
      false,
    ...
    assert.strictEqual(
      await UnChurchBoolean(
        await arenaEvaluator.reduce(apply(IsZero, ChurchN(0))),
        arenaEvaluator,
      ),
      true,
    );
    ...
    assert.strictEqual(
      await UnChurchBoolean(
        await arenaEvaluator.reduce(apply(IsZero, ChurchN(1))),
        arenaEvaluator,
      ),
      false,
    );
    ...
    for (let m = 0n; m < N; m++) {
      for (let n = 0n; n < N; n++) {
        ...
        assert.strictEqual(
          await UnChurchNumber(
            await arenaEvaluator.reduce(
              applyMany(ChurchN(m), Succ, ChurchN(n)),
            ),
            arenaEvaluator,
          ),
          m + n,
        ...
        assert.strictEqual(
          await UnChurchNumber(
            await arenaEvaluator.reduce(
              applyMany(Plus, ChurchN(m), ChurchN(n)),
            ),
            arenaEvaluator,
          ),
          m + n,
